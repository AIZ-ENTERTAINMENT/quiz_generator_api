...
@router.post("/")
async def create_prompt(prompt_content: str):
    "create prompt"
    res = await service.create_prompt(prompt_content)
...
async def update_prompt(prompt_id: int, prompt_content: str):
    "update prompt" 
    await service.update_prompt(prompt_id, prompt_content)
